/*
 * elu kernel test — element-wise ELU(x) = x if x>=0 else e^x - 1
 *
 * Uses hardware fexp.s instruction, branchless kernel.
 */
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
...
int main() {
    ...
    size_t ksize;
    load_kernel(vram, "bin/kernel/elu.bin", &ksize);
    ...
    for (int i = 0; i < N; i++) {
        ...
        expected[i] = input[i] >= 0 ? input[i] : expf(input[i]) - 1.0f;
    }
