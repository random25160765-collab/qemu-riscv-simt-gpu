/*
 * gelu kernel test — element-wise GELU(x) = x * sigmoid(1.702*x)
 *
...
int main() {
    ...
    size_t ksize;
    load_kernel(vram, "bin/kernel/gelu.bin", &ksize);
    ...
    for (int i = 0; i < N; i++) {
        ...
        float s = 1.0f / (1.0f + expf(-1.702f * input[i]));
