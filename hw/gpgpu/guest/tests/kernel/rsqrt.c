/*
 * rsqrt kernel test — element-wise rsqrt(x) = 1/sqrt(x)
 *
 * Uses hardware frsqrt.s instruction.
 * Input must be positive; we use fabsf to ensure that.
 */
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
...
int main() {
    ...
    size_t ksize;
    load_kernel(vram, "bin/kernel/rsqrt.bin", &ksize);
    printf("Loaded kernel: %zu bytes\n", ksize);

    float input[N], expected[N];
    srand(time(NULL));
    for (int i = 0; i < N; i++) {
        input[i] = (float)(rand() % 100 + 1) / 10.0f;  /* positive */
        expected[i] = 1.0f / sqrtf(input[i]);
    }
