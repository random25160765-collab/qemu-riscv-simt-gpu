/*
 * saxpy kernel test — C[i] = alpha * A[i] + B[i]
 *
...
int main() {
    ...
    size_t ksize;
    load_kernel(vram, "bin/kernel/saxpy.bin", &ksize);
    ...
    /* write alpha to VRAM — the kernel reads 0x400000 */
    *(volatile float *)(vram + ALPHA_OFFSET) = alpha;
    ...
    for (int i = 0; i < N; i++) {
        if (fabsf(result[i] - expected[i]) > TOLERANCE) {
            if (errors < MAX_SHOW)
                printf("  [%3d] A=%.4f B=%.4f alpha=%.4f expected=%.4f got=%.4f  FAIL\n",
                   i, A[i], B[i], alpha, expected[i], result[i]);
            errors++;
        }
    }
    if (errors > MAX_SHOW)
