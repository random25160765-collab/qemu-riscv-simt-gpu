/*
 * vector_add kernel test — element-wise vector add C[i] = A[i] + B[i]
 *
 * This is the original vector_add kernel, tested at the kernel level.
 */
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
...
int main() {
    ...
    size_t ksize;
    load_kernel(vram, "bin/kernel/vector_add.bin", &ksize);
    printf("Loaded kernel: %zu bytes\n", ksize);

    float A[N], B[N], expected[N];
    srand(time(NULL));
    for (int i = 0; i < N; i++) {
        A[i] = (float)(rand() % 200 - 100) / 10.0f;
        B[i] = (float)(rand() % 200 - 100) / 10.0f;
        expected[i] = A[i] + B[i];
    }
    ...
    for (int i = 0; i < N; i++) {
        if (fabsf(result[i] - expected[i]) > TOLERANCE) {
            if (errors < MAX_SHOW)
                printf("  [%3d] A=%.4f B=%.4f expected=%.4f got=%.4f  FAIL\n",
                   i, A[i], B[i], expected[i], result[i]);
            errors++;
        }
    }
    if (errors > MAX_SHOW)
