/*
 * vecdiv kernel test — element-wise vector divide C[i] = A[i] / B[i]
 */
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
...
int main() {
    ...
    size_t ksize;
    load_kernel(vram, "bin/kernel/vecdiv.bin", &ksize);
    ...
    for (int i = 0; i < N; i++) {
        ...
        B[i] = (float)(rand() % 100 + 1) / 10.0f;  /* avoid zero */
        expected[i] = A[i] / B[i];
    }
