...
int decode_trie_build(DecodeTrie *trie,
                      const opcode_entry_t *entries, size_t count)
{
...
static int count_nodes_recur(const DecodeTrie *trie, uint16_t idx)
{
    ...
    return 1 + count_nodes_recur(trie, trie->nodes[idx].child[0])
             + count_nodes_recur(trie, trie->nodes[idx].child[1]);
}
...
{
    if (!trie || !trie->nodes) { printf("  TRIE: (empty)\n"); return; }
