...
int gpgpu_core_simd_exec_warp(GPGPUState *s, GPGPUWarp *warp,
    ...
    SIMD_RESOLVE_HANDLERS(code, tcount);
    goto *ip++->handler;
    ...
    op_lw:  { int rd=R.rd,rs1=R.rs1,imm=R.imm; S G(rd,i)=gpu_read(s,G(rs1,i)+imm,4); S PC(i)+=4; goto *ip++->handler; }
    ...
    /* ebreak */
    op_ebreak: { for(int i=0;i<32;i++){warp->lanes[i].pc=PC(i);for(int r=0;r<GPGPU_NUM_REGS;r++){warp->lanes[i].gpr[r].u32=G(r,i);warp->lanes[i].fpr[r].u32=F(r,i);}} return 0; }
    op_done: { goto op_ebreak; }
    /* FP — 标量 */
    op_flw: { int rd=R.rd,rs1=R.rs1,imm=R.imm; for(int i=0;i<32;i++) F(rd,i)=gpu_read(s,G(rs1,i)+imm,4); S PC(i)+=4; goto *ip++->handler; }
    op_fsw: { int rs1=R.rs1,rs2=R.rs2,imm=R.imm; S gpu_write(s,G(rs1,i)+imm,4,F(rs2,i)); S PC(i)+=4; goto *ip++->handler; }
    #define FPS(n,body) op_##n: { FP_SYNC(l); body; FP_BACK(l); l->pc+=4; goto *ip++->handler; }
    FPS(fadd_s,l->fpr[R.rd].u32=float32_add(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(fsub_s,l->fpr[R.rd].u32=float32_sub(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(fmul_s,l->fpr[R.rd].u32=float32_mul(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(fdiv_s,l->fpr[R.rd].u32=float32_div(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(fsqrt_s,l->fpr[R.rd].u32=float32_sqrt(l->fpr[R.rs1].u32,&l->fp_status))
    FPS(fmadd_s,l->fpr[R.rd].u32=float32_muladd(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,l->fpr[R.rs3].u32,0,&l->fp_status))
    FPS(fmsub_s,l->fpr[R.rd].u32=float32_muladd(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,l->fpr[R.rs3].u32,float_muladd_negate_c,&l->fp_status))
    FPS(fnmsub_s,l->fpr[R.rd].u32=float32_muladd(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,l->fpr[R.rs3].u32,float_muladd_negate_product,&l->fp_status))
    FPS(fnmadd_s,l->fpr[R.rd].u32=float32_muladd(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,l->fpr[R.rs3].u32,float_muladd_negate_result,&l->fp_status))
    FPS(fsgnj_s,l->fpr[R.rd].u32=(l->fpr[R.rs1].u32&~0x80000000)|(l->fpr[R.rs2].u32&0x80000000))
    FPS(fsgnjn_s,l->fpr[R.rd].u32=(l->fpr[R.rs1].u32&~0x80000000)|((~l->fpr[R.rs2].u32)&0x80000000))
    FPS(fsgnjx_s,l->fpr[R.rd].u32=l->fpr[R.rs1].u32^(l->fpr[R.rs2].u32&0x80000000))
    FPS(fmin_s,l->fpr[R.rd].u32=float32_min(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(fmax_s,l->fpr[R.rd].u32=float32_max(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(feq_s,l->gpr[R.rd].u32=float32_eq(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(flt_s,l->gpr[R.rd].u32=float32_lt(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(fle_s,l->gpr[R.rd].u32=float32_le(l->fpr[R.rs1].u32,l->fpr[R.rs2].u32,&l->fp_status))
    FPS(fcvt_s_w,l->fpr[R.rd].u32=int32_to_float32(l->gpr[R.rs1].i32,&l->fp_status))
    FPS(fcvt_s_wu,l->fpr[R.rd].u32=uint32_to_float32(l->gpr[R.rs1].u32,&l->fp_status))
    FPS(fmv_w_x,l->fpr[R.rd].u32=l->gpr[R.rs1].u32) FPS(fmv_x_w,l->gpr[R.rd].u32=l->fpr[R.rs1].u32)
    ...
    FPS(fcvt_s_e4m3,l->fpr[R.rd].u32=e4m3_to_f32(l->fpr[R.rs1].e4m3)) FPS(fcvt_e4m3_s,l->fpr[R.rd].e4m3=f32_to_e4m3(l->fpr[R.rs1].u32))
    FPS(fcvt_s_e5m2,l->fpr[R.rd].u32=e5m2_to_f32(l->fpr[R.rs1].e5m2)) FPS(fcvt_e5m2_s,l->fpr[R.rd].e5m2=f32_to_e5m2(l->fpr[R.rs1].u32))
    FPS(fcvt_s_e2m1,l->fpr[R.rd].u32=e2m1_to_f32(l->fpr[R.rs1].e2m1)) FPS(fcvt_e2m1_s,l->fpr[R.rd].e2m1=f32_to_e2m1(l->fpr[R.rs1].u32))
    ...
    #undef SCI
    #undef FPS
    op_illegal: { for(int i=0;i<32;i++){warp->lanes[i].pc=PC(i);for(int r=0;r<GPGPU_NUM_REGS;r++){warp->lanes[i].gpr[r].u32=G(r,i);warp->lanes[i].fpr[r].u32=F(r,i);}} return -1; }
}
