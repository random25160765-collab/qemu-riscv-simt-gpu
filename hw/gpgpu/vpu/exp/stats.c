...
void stats_reset(void) { n_entries = 0; }
...
{
    if (v < 1024)           snprintf(buf, sz, "%.0f", v);
    else if (v < 1024*1024) snprintf(buf, sz, "%.1fK", v/1024);
    else                    snprintf(buf, sz, "%.1fM", v/1e6);
    return buf;
}
...
static char *fmt_mix(char *buf, size_t sz, const uint64_t cat[4])
{
    ...
    /* fixed-width: ALU:XX% FP:XX% MEM:XX% BR:XX% = 4×(4+4) = 32 chars max */
    snprintf(buf, sz, "%s:%2.0f%% %s:%2.0f%% %s:%2.0f%% %s:%2.0f%%",
             cn[0], 100.0*cat[0]/tot,
             cn[1], 100.0*cat[1]/tot,
             cn[2], 100.0*cat[2]/tot,
             cn[3], 100.0*cat[3]/tot);
    return buf;
}
...
void stats_render(void)
{
    ...
    for (int i = 0; i < n_entries; i++) {
        ...
        uint64_t mt = 0; for (int c = 0; c < 4; c++) mt += e->cat[c] + e->cat_static[c];
    ...
    if (has_mix) printf("  %-32s", "mix");
    if (has_div) printf(" %8s", "div");
    printf(" %s\n", "");
    ...
    if (has_mix) printf("  %-32s", "--------------------------------");
    if (has_div) printf(" %8s", "--------");
    printf(" %s\n" KNRM, "----");
    ...
    for (int i = 0; i < n_entries; i++) {
        ...
        if (has_bw) {
            if (e->bytes_r + e->bytes_w > 0) {
                ...
                printf("  " KBLU "r:" KNRM "%s " KBLU "w:" KNRM "%s %6.0fM/s",
                       fmt_n(rn, sizeof(rn), e->bytes_r),
        ...
        if (has_mix) {
            ...
            snprintf(tag, sizeof(tag), "%s%s",
                     is_dyn ? KCYN "d" KNRM : KDIM "s" KNRM,
                     is_dyn ? "yn " : "t ");
            printf("  %s%s", tag, mx);
        }
